import pandas as pd
import numpy as np
from sklearn.feature_extraction.text import TfidfVectorizer  # Fixed capitalization here
...
class CarRecommendationSystem:
    """
    A comprehensive recommendation system for Mercedes-Benz that implements both
    ...
    def get_hybrid_recommendations(self, user_id, vehicle_id=None, content_weight=0.5, top_n=5):
        """
        ...
        user_id : int or str
            ID of the user to get recommendations for
        vehicle_id : int or str, optional
            ID of a vehicle to use as a reference point for content-based filtering
        content_weight : float
            Weight to give content-based recommendations (between 0 and 1)
        top_n : int
            Number of recommendations to return
        ...
        try:
            cf_recommendations = self.get_collaborative_filtering_recommendations(user_id, top_n=top_n * 2)

            # If vehicle_id is provided, use it for content-based recommendations
            if vehicle_id is not None:
                cb_recommendations = self.get_content_based_recommendations(vehicle_id, top_n=top_n * 2)

                # Combine the recommendations with weighted scores
                cf_recs_dict = {row['vehicle_id']: row['predicted_rating']
                                for _, row in cf_recommendations.iterrows()}

                cb_recs_dict = {row['vehicle_id']: row['similarity_score']
                                for _, row in cb_recommendations.iterrows()}

                # Normalize the scores to be in the same range
                if cf_recs_dict:
                    max_cf = max(cf_recs_dict.values())
                    min_cf = min(cf_recs_dict.values())
                    cf_range = max_cf - min_cf if max_cf > min_cf else 1
                    cf_recs_dict = {k: (v - min_cf) / cf_range for k, v in cf_recs_dict.items()}

                if cb_recs_dict:
                    max_cb = max(cb_recs_dict.values())
                    min_cb = min(cb_recs_dict.values())
                    cb_range = max_cb - min_cb if max_cb > min_cb else 1
                    cb_recs_dict = {k: (v - min_cb) / cb_range for k, v in cb_recs_dict.items()}

                # Combine the scores
                combined_scores = {}
                all_vehicle_ids = set(cf_recs_dict.keys()) | set(cb_recs_dict.keys())

                for vid in all_vehicle_ids:
                    cf_score = cf_recs_dict.get(vid, 0)
                    cb_score = cb_recs_dict.get(vid, 0)
                    combined_scores[vid] = (1 - content_weight) * cf_score + content_weight * cb_score

                # Get top_n vehicles with highest combined scores
                top_vehicle_ids = sorted(combined_scores.keys(),
                                         key=lambda x: combined_scores[x],
                                         reverse=True)[:top_n]

                # Create a DataFrame with the recommended vehicles
                recommended_vehicles = self.vehicle_data[self.vehicle_data['vehicle_id'].isin(top_vehicle_ids)].copy()
                recommended_vehicles['hybrid_score'] = recommended_vehicles['vehicle_id'].map(combined_scores)
                recommended_vehicles = recommended_vehicles.sort_values('hybrid_score', ascending=False)

            else:
                # If no vehicle_id provided, just return collaborative filtering recommendations
                recommended_vehicles = cf_recommendations.head(top_n)

            logger.info(f"Found {len(recommended_vehicles)} hybrid recommendations")
            return recommended_vehicles
...
if __name__ == "__main__":
    ...
    try:
        ...
        hybrid_recs = recsys.get_hybrid_recommendations(user_id, vehicle_id)
        recsys.visualize_recommendations(hybrid_recs, "Hybrid Recommendations")

        # Save the results
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        recsys.save_recommendations_to_file(hybrid_recs, f"hybrid_{timestamp}.csv")
